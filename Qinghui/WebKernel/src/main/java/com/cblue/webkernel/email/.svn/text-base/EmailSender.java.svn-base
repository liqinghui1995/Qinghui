package com.cblue.webkernel.email;
...
public class EmailSender {
    ...
    private static final String SENDEMAIL_ADDRESS="";  //发件箱地址
    private static final String SENDEMAIL_USERNAME="";   //发件箱用户名
    private static final String SENDEMAIL_USERPASSWORD=""; //发件箱密码
    private static final String SMTP_ADDRESS="smtp.163.com"; //stmp服务器地址
    ...
    public static void send(String[] receivers, String subject, String mailContent, File[] attachements, String mimetype) {
        ...
        props.put("mail.smtp.host", SMTP_ADDRESS);//smtp服务器地址 
        props.put("mail.smtp.auth", "true");//需要校验
        Session session = Session.getDefaultInstance(props, new Authenticator() {
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication(SENDEMAIL_USERNAME,SENDEMAIL_USERPASSWORD);//登录用户名/密码
            }
        });
        session.setDebug(true);
        try {
            MimeMessage mimeMessage = new MimeMessage(session);
            mimeMessage.setFrom(new InternetAddress(SENDEMAIL_ADDRESS));//发件人邮件
